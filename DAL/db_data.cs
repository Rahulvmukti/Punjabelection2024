...
namespace exam.DAL
{
    public class db_data
    {
        ...
        public DataSet GetMapBoothList(string usertype, bool isgrid, int st_id)
        {
            ...
            try
            {
                ...
                query = " select  b.district,s.streamname as videoname,s.servername as cdnsvc,s.status," +
                        ...
                        " inner join streamlist s on b.streamid = s.id " +
                        "inner join district d on d.district=b.district" +
                        " where ISNULL(b.longitude,0) != 0 and ISNULL(b.latitude,0) != 0 and b.isdisplay = 'True'" +
                        " and ((@Parliament ='') OR(d.Parliament=@Parliament))" +
                        " and ((@dst='') OR (b.district=@dst)) and s.IsEnable = 1" +
                        " and b.boothstateid = '1'   group by b.district,streamname,servername,status,b.acname,PSNum,b.longitude,b.latitude,b.id," +
                        " b.longitude,b.latitude ,b.location,s.hasalarm Order By s.hasalarm desc, len(s.streamname) asc; " +
                        ...
                        " l.longitude,l.latitude,l.UpdateDatetime from booth b inner join district d on d.district=b.district and b.accode=d.accode  full join location_info l on b.id = l.boothid  inner join streamlist s on b.streamid = s.id " +
                        " where  b.isdisplay = 'True' and ((@Parliament ='') OR(d.Parliament=@Parliament))and ((@dst='') OR (b.district=@dst))  and s.IsEnable = 1 and b.boothstateid = '1'" +
                        " group by b.district,streamname,servername,status,b.acname,PSNum,b.longitude,b.latitude,b.id,l.longitude,l.latitude,l." +
                        " UpdateDatetime ,b.location,s.hasalarm) tbl";
                SqlCommand command = new SqlCommand(query, conn);
                command.Parameters.AddWithValue("@dst", dist);
                command.Parameters.AddWithValue("@Parliament", parliment);
        ...
        public DataSet GetMapBoothListNew_Grid_fategarh(int userid, string district, string assembly, int statusflag, string status, int isPink = -1, int isARO = -1, string booth = "", string streamname = "", string orderBy = "", int PageNumber = 1, int pageitemcount = 6)
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
            {
                ...
                SqlCommand command = new SqlCommand("GetBoothListNew12", conn);
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.AddWithValue("@district", district);
                command.Parameters.AddWithValue("@assembly", assembly);
                command.Parameters.AddWithValue("@status", status);
        ...
        public DataSet GetDistrictList(string usertype, int st_id)
        {
            ...
            try
            {
                ...
                //{
                query = "SELECT distinct(b.district)district,b.district as SelValue FROM " + table_prefix + "booth b with(nolock) inner join " + table_prefix + "streamlist s with(nolock) on b.streamid=s.id WHERE Isdisplay=1 and b.boothstateid=" + st_id + " ORDER BY b.District ASC;";
                //}
        ...
        public DataSet GetDistrictListECI(string usertype, int st_id)
        {
            ...
            try
            {
                ...
                if (usertype == "Master_Admin" || usertype == "live")
                {
                    query = "SELECT distinct(b.district)district,b.district as SelValue  FROM " + table_prefix + "booth b with(nolock) inner join " + table_prefix + "streamlist s with(nolock) on b.streamid=s.id WHERE Isdisplay=1 and b.boothstateid='" + st_id + "' and s.selected=1 ORDER BY b.District ASC;";
                }
                else if (usertype.Contains("?"))
                {
                    utype = usertype.Split('?');
                    ...
                    if (utype[0] == "admin" || utype[0] == "live")
                    {
                        query = "SELECT distinct(b.district)district,b.district as SelValue FROM " + table_prefix + "booth" + tblname + " b with(nolock) inner join " + table_prefix + "streamlist" + tblname + " s with(nolock) on b.streamid=s.id WHERE Isdisplay=1 and b.boothstateid='" + utype[1] + "' ORDER BY b.District ASC;";
                    }

                    else
                    {
                        dist = utype[0].Split('_')[1];
                        query = "SELECT distinct(b.district)district,b.district as SelValue FROM Booth" + tblname + " b with(nolock) inner join streamlist" + tblname + " s with(nolock) on b.streamid=s.id WHERE b.Isdisplay=1 and b.district =N'" + dist + "' and b.boothstateid='" + utype[1] + "' ORDER BY b.District ASC;";
                        //query = "SELECT distinct([acname]) FROM [Booth] WHERE Isdisplay=1 and district ='" + dist + "'  ORDER BY [schoolname] ASC;";
                    }
                ...
                {
                    dist = usertype.Split('_')[1];
                    query = "SELECT distinct(b.district)district,b.district as SelValue FROM " + table_prefix + "booth b with(nolock) inner join streamlist s with(nolock) on b.streamid=s.id WHERE b.Isdisplay=1 and b.district  in (select zoneDistrict from " + table_prefix + "zoneinfo where zoneName ='" + dist + "')  and b.boothstateid='" + st_id + "' ORDER BY b.District ASC;";

                }
                else
                {
                    dist = usertype.Split('_')[1];
                    query = "SELECT distinct(b.district)district,b.district as SelValue FROM " + table_prefix + "booth b with(nolock) inner join streamlist s with(nolock) on b.streamid=s.id WHERE b.Isdisplay=1 and b.district = N'" + dist + "' and b.boothstateid='" + st_id + "' and s.selected=1 ORDER BY b.District ASC;";
                    //query = "SELECT distinct([acname]) FROM [Booth] WHERE Isdisplay=1 and district ='" + dist + "'  ORDER BY [schoolname] ASC;";
                }
        ...
        public void updateloginstatus(bool status, string username)
        {
            ...
            try
            {
                // Use a CASE statement to ensure logincount does not decrement below 0
                string query = "UPDATE " + table_prefix + "users SET islogin = @islogin, " +
                               "logincount = CASE WHEN logincount > 0 THEN logincount - 1 ELSE 0 END " +
        ...
        public DataSet GetAllDistrictByStateId(int stateid)
        {
            ...
            try
            {
                ...
                query = "select district,id from(SELECT district,id, ROW_NUMBER() over(partition by district order by district) rn  from " + table_prefix + ".[district] with(nolock) WHERE stateid=" + stateid + " ) as tbl where rn=1";
        ...
        public DataSet GetAllAssemblyByDistrict(int stateid, string district)
        {
            ...
            {
                string query = string.Empty;
                query = "SELECT accode,acname  from " + table_prefix + "district with(nolock) WHERE stateid=" + stateid + " AND district='" + district + "' ORDER BY District ASC;";
                SqlCommand command = new SqlCommand(query, conn);
                SqlDataAdapter da = new SqlDataAdapter(command);
                da.Fill(ds);
                ds = FilterDataByAccess(ds, false, true);
        ...
        public DataSet GetIndoorOutDoorForGraphAssemblyWise(string district, string accode, int UserID)
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            ...
            {
                string query = string.Empty;
                query = "GetIndoorOutDoorForGraphAssemblyWise";
                SqlCommand command = new SqlCommand(query, conn);
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.AddWithValue("@lastseen", chklastseendt);
                command.Parameters.AddWithValue("@district", district);
        ...
        public DataSet GetCameraStatusReport(string FromDt, string ToDt, string district, string accode, string vehicleNo)
        {
            ...
            {
                string query = string.Empty;
                query = "[dbo].[GetCameraStatusReport3]";
                SqlCommand command = new SqlCommand(query, conn);
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.AddWithValue("@district", district);
                command.Parameters.AddWithValue("@accode", accode);
            ...
            {
                Common.Log("GetCameraStatusReport()--> " + ex.Message);
            }
            finally
            {
                conn.Close();
        ...
        public DataSet GetExcelReport(string FromDt, string ToDt, string district, string accode, string vehicleNo, int islive, string status, string streamname, string userid)
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
        ...
        public DataSet getinsatalled(string FromDt, string ToDt, string district, string accode, string vehicleNo, int islive, string status, string streamname, string userid)
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
        ...
        public DataSet getuninstalled(string FromDt, string ToDt, string district, string accode, string vehicleNo, int islive, string status, string streamname, string userid)
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
            {
                string query = string.Empty;
                query = "[dbo].[GetuninstalledReport]";
                SqlCommand command = new SqlCommand(query, conn);
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.AddWithValue("@district", district);
                command.Parameters.AddWithValue("@accode", accode);
        ...
        public DataSet GetBLOInformation()
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
            {
                SqlCommand command = new SqlCommand("GetBLOInformation", conn);
                command.CommandType = CommandType.StoredProcedure;
                SqlDataAdapter da = new SqlDataAdapter(command);
                da.Fill(ds);
            }
            ...
            {
                Common.Log("GetBLOInformation()--> " + ex.Message);
            }
            finally
            {
        ...
        //get unmappedlist
        public DataSet Getunmappedlist()
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
            {
                SqlCommand command = new SqlCommand("getunmappedlist", conn);
                command.CommandType = CommandType.StoredProcedure;
                SqlDataAdapter da = new SqlDataAdapter(command);
                da.Fill(ds);
            }
        ...
        //    return ds;
        //}
        public int SaveStreamList(string deviceid, string servername, string prourl, string userName, string PageName, int StreamID = 0)
        {
            DataSet ds = new DataSet();
            SqlConnection SQLconn = new SqlConnection(connstr);
            try
            {
                ...
                SQLcommand.Parameters.Add(new SqlParameter("@StreamID", StreamID));
                SQLconn.Open();
        ...
        public DataSet GetMapBoothListNew_Master(string district, string assembly, string Search, string cameratype, int userid)
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
        ...
        public DataSet Getdistrictid(int id)
        {
            ...
            try
            {
                ...
                WHERE id IN (
                    ...
                    FROM STRING_SPLIT((SELECT AssemblyAccesIds FROM users with(nolock) WHERE usercode = 'District_Level' AND id <> 56 AND id = @UserId), ',')
                )";
        ...
        public DataSet GetMapBoothListNew_Grid(int userid, string district, string assembly, int statusflag, string locationtype, string status, int isPink = -1, int isARO = -1, string booth = "", string streamname = "", string orderBy = "", int PageNumber = 1, int pageitemcount = 6)
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
        ...
        //outdoor grid
        public DataSet GetMapBoothListNewoutdoor_Grid(int userid, string district, string assembly, int statusflag, string status, int isPink = -1, int isARO = -1, string booth = "", string streamname = "", string orderBy = "", int PageNumber = 1, int pageitemcount = 6)
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
            {
                ...
                SqlCommand command = new SqlCommand("GetBoothListNew305", conn);
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.AddWithValue("@district", district);
                command.Parameters.AddWithValue("@assembly", assembly);
                command.Parameters.AddWithValue("@status", status);
        ...
        ///Video size calculation method
        public DataSet GetVideoSize(string district, string assembly, string LoactionType, string status, int isPink = -1, int isARO = -1, string booth = "", string streamname = "", string orderBy = "")
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
            {
                ...
                SqlCommand command = new SqlCommand("GetServerSize", conn);
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.AddWithValue("@district", district);
                command.Parameters.AddWithValue("@assembly", assembly);
                command.Parameters.AddWithValue("@LocationType", LoactionType);
        ...
        public DataSet Parliament(string usertype)
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
            {
                ...
                SqlCommand command = new SqlCommand(query, conn);
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.AddWithValue("@flag", "getalldata");
                SqlDataAdapter da = new SqlDataAdapter(command);
        ...
        //districtbyparliment load

        public DataSet GetDistrictbyparliment(string usertype, string parliment)
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
            {
                ...
                SqlCommand command = new SqlCommand(query, conn);
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.AddWithValue("@parliment", parliment);
                command.Parameters.AddWithValue("@flag", "getdist");
                SqlDataAdapter da = new SqlDataAdapter(command);
        ...
        public DataSet GetACbyparlimentDistrict(string usertype, string parliment, string district)
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
            {
                ...
                command.Parameters.AddWithValue("@parliment", parliment);
                command.Parameters.AddWithValue("@District", district);
                command.Parameters.AddWithValue("@flag", "getassembly");
                SqlDataAdapter da = new SqlDataAdapter(command);
        ...
        //user page tables
        public bool UpdateUserSessionActivityByUserId(int userid, string sessionid)
        {
            DataSet ds = new DataSet();
            SqlConnection SQLconn = new SqlConnection(connstr);
            try
        ...
        public DataSet GetCameraOfflineList11(string FromDt, string ToDt, string district, string accode, string vehicleNo, string shift)
        {
            ...
            try
            {
                ...
                command.Parameters.AddWithValue("@district", district);
                command.Parameters.AddWithValue("@accode", accode);
                command.Parameters.AddWithValue("@location", vehicleNo);
                command.Parameters.AddWithValue("@FromDt", FromDt);
                command.Parameters.AddWithValue("@ToDt", ToDt);
                command.CommandTimeout = 240;
                SqlDataAdapter da = new SqlDataAdapter(command);
        ...
        public DataSet GetStaticacname(string usertype, int UserID, string cameraType = "", int pink = -1)
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
            {
                ...
                string query = string.Empty;
                query = "GetHeatMapCount_Assembly_Online_ConnectedOnce";
                SqlCommand command = new SqlCommand(query, conn);
                command.Parameters.AddWithValue("@lastseen", chklastseendt);
        ...
        public DataSet GetdistrictStaticname(string usertype, int UserID, string cameraType = "", int pink = -1)
        {
            DataSet ds = new DataSet();
            SqlConnection conn = new SqlConnection(connstr);
            try
            {
                ...
                string query = string.Empty;
                query = "GetHeatMapCount_District_Online_ConnectedOnce";
                SqlCommand command = new SqlCommand(query, conn);
                command.Parameters.AddWithValue("@lastseen", chklastseendt);
                command.Parameters.AddWithValue("@lastseendefault", chklastseendefaultdt);
                command.Parameters.AddWithValue("@UserID", UserID);
