    ...
    public class db_data_admin
    {
        ...
        public DataSet GetDistrictList(string user_type, int st_id)
        {
            ...
            try
            {
                ...
                if (user_type == "admin")
                {
                    query = "SELECT distinct(district), district as SelValue  FROM " + table_prefix + "district with(nolock) where stateid=@stateid ORDER BY District ASC;";
                }
                else
                {
                    ...
                    query = "SELECT distinct(district),district as SelValue  FROM " + table_prefix + "district with(nolock) where district=N'" + user_type + "' and stateid=@stateid ORDER BY District ASC;";
                }
                SqlCommand command = new SqlCommand(query, conn);
                command.Parameters.AddWithValue("@stateid", st_id);
                SqlDataAdapter da = new SqlDataAdapter(command);
