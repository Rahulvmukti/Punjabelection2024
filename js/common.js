...
function LoadPlayer(playerid, sname, plid, schname, accode, psnum, kbps, mobno) {
    ...
    if (playerType == "videoplayer") {
        ...
        if (playerid.indexOf("_") >= 0) {
            ...
            var url = 'https://' + sname + '/live-record/' + playeridurl + '.flv';
        }
        else {
            var url = 'https://' + sname + '/live-record/' + playerid + '.flv';
        }
        ...
        var flvPlayer = flvjs.createPlayer({
            ...
            autoCleanupMaxBackwardDuration: 5000,
        ...
        LoadNodePlayer(playerid, sname, plid, schname )
    }
    return false;
}

function LoadNodePlayer(playerid, sname, plid, schname) {
    // Set the 'id' attribute for the element with the class "ptzcanvas"
    $(".ptzcanvas").attr("id", playerid);
    ...
    canv.id = playerid;
    console.log('===================>2222222222' + playerid);
    
    // Append the canvas element to the divptz1
    var divptz = document.getElementById('divptz1'); // Assuming divptz1 is an existing element
    divptz.appendChild(canv);

    var player;
    var interval = null;
    var playeridurl = playerid.split('_')[1];

    NodePlayer.load(() => {
        // Create a player instance
        player = new NodePlayer();
        debugger
       // player.setView(playeridurl);

        var customURL = "wss://" + sname + ":443/live-record/" + playeridurl + ".flv";
        ...
        // Start the player with the custom URL
        player.start(customURL);
    });
}
